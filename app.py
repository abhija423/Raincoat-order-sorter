        ...
        for page in matched_pages:
            ...
            for group in groups:
                if identity_key == (group[0]["identity"][0], group[0]["identity"][2]):
                    # Check the layout address similarity rule
...
# --- Core Engine Logic Hook ---
if uploaded_files and process_clicked:
    # 📊 1. Create centralized, uniform progress UI layout
    progress = st.progress(0)
    status = st.empty()
    eta = st.empty()

    status.markdown("### 🧬 Initializing Engines\nCombining target file segments...")
    ...
    # Pass extraction state to structural worker
    ...
    # 🚀 6. Generate Main and Duplicate together in ONE loop pass
    ...
    for i, page in enumerate(main_pages):
        # 🚀 4. Throttle state updates to every 20 loops
        if i % 20 == 0 or i == len(main_pages) - 1:
            percent = (i + 1) / total_compilation
            progress.progress(percent)
            # 📊 2. Show active count tracking
            status.markdown(
                f"### 📄 Generating Output Repositories\n"
                f"**Processed (Main):** {i+1:,} / {len(main_pages):,} pages"
            )
    ...
    # Pack Duplicate orders in continuation sequence
    offset = len(main_pages)
    for i, page in enumerate(duplicate_pages):
        ...
        if i % 20 == 0 or i == len(duplicate_pages) - 1:
            ...
            status.markdown(
                f"### 👥 Generating Output Repositories\n"
                f"**Processed (Duplicates):** {i+1:,} / {len(duplicate_pages):,} pages"
            )
            elapsed = time.perf_counter() - start_compilation
    ...
    # 🚀 5. Generate Cropped PDF using chunked 20-page metrics
    ...
    # Clear Progress HUD components prior to UI state refresh
    progress.empty()
    status.empty()
    eta.empty()
