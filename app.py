...
def show_debug_table(main_pages, duplicate_pages, exchange_orders, bulk_orders):
    ...
    st.write(f"Exchange Orders : {len(exchange_orders)}")
    st.write(f"Bulk Orders : {len(bulk_orders)}")
    st.write(f"Duplicate PDF Pages : {len(duplicate_pages)}")
...
def show_metrics(normal_orders, exchange_orders, bulk_orders, duplicate_groups):
    ...
    c1.metric("Normal", len(normal_orders))
    c2.metric("Exchange", len(exchange_orders))
    c3.metric("Bulk Qty", len(bulk_orders))
    c4.metric("Duplicate Pages", duplicate_pages)
...
if uploaded_file is not None:
    ...
    show_metrics(normal_orders, exchange_orders, bulk_orders, duplicate_groups)
